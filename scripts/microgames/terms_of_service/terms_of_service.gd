...
onready var v_scroll_bar : VScrollBar = scroll_container.get_v_scrollbar()
...
func _ready() -> void:
	...
	v_box_container.rect_min_size = Vector2.DOWN * scroll_length[difficulty - 1]
	the_end_of_bar = int(v_box_container.rect_min_size.y) - 376
...
func set_up_articles() -> void:
	...
	for i in v_box_container.get_child_count():
		...
		rich_text_child.rect_position.y = prev_y_pos + (prev_offset_text * FONT_SEPARATION) + (FONT_SEPARATION if i != 0 else 0)
		
		prev_y_pos = rich_text_child.rect_position.y
...
func update_tos_text(article_index : int) -> void:
	...
	cur_article_child.rect_position.y = cur_article_pos_offset
	
	cur_article_pos_offset = cur_article_child.rect_position.y + (cur_article_child.get_line_count() * FONT_SEPARATION) + FONT_SEPARATION
...
func get_tos_text(article_count : int) -> PoolStringArray:
	...
	var temp_array : PoolStringArray = text_file.split("[br][br]", false)
