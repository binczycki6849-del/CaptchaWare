...
export(String, "normal", "absurd", "all", "campaign") var cur_microgame_pool : String = "all"
...
func transition_game() -> void:
	...
	var did_fail = prev_microgame != null && !prev_microgame.isWinning()
...
func set_judgement_text(failed = false) -> void:
	var dialogue = ""
	if failed:
		dialogue = judgement_text.lose_dialogue[randi() % judgement_text.lose_dialogue.size()]
	else:
		dialogue = judgement_text.win_dialogue[randi() % judgement_text.win_dialogue.size()]
...
func _on_captcha_animation_player_animation_finished(anim_name) -> void:
	...
	"gametransition_boss"].has(str(anim_name)): return
